...
const addTagToCategory = `-- name: AddTagToCategory :exec
INSERT INTO category_tags (category_id, tag_id)
VALUES ($1, $2)
ON CONFLICT DO NOTHING
`

type AddTagToCategoryParams struct {
	CategoryID int64 `json:"categoryId"`
	TagID      int64 `json:"tagId"`
}

func (q *Queries) AddTagToCategory(ctx context.Context, arg AddTagToCategoryParams) error {
	_, err := q.db.Exec(ctx, addTagToCategory, arg.CategoryID, arg.TagID)
	return err
}
...
const getTagIdsByCategoryID = `-- name: GetTagIdsByCategoryID :many
SELECT ct.category_id, ct.tag_id
FROM tags t
...
func (q *Queries) GetTagIdsByCategoryID(ctx context.Context, categoryID int64) ([]CategoryTag, error) {
	rows, err := q.db.Query(ctx, getTagIdsByCategoryID, categoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []CategoryTag
	for rows.Next() {
		var i CategoryTag
		if err := rows.Scan(&i.CategoryID, &i.TagID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const getTagIdsByCategoryIds = `-- name: GetTagIdsByCategoryIds :many
SELECT ct.category_id, ct.tag_id
FROM tags t
         JOIN category_tags ct ON ct.tag_id = t.id
...
func (q *Queries) GetTagIdsByCategoryIds(ctx context.Context, dollar_1 []int64) ([]CategoryTag, error) {
	rows, err := q.db.Query(ctx, getTagIdsByCategoryIds, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const removeTagFromCategory = `-- name: RemoveTagFromCategory :exec
DELETE
FROM category_tags
WHERE category_id = $1
  AND tag_id = $2
...
type RemoveTagFromCategoryParams struct {
	CategoryID int64 `json:"categoryId"`
	TagID      int64 `json:"tagId"`
}

func (q *Queries) RemoveTagFromCategory(ctx context.Context, arg RemoveTagFromCategoryParams) error {
	_, err := q.db.Exec(ctx, removeTagFromCategory, arg.CategoryID, arg.TagID)
	return err
}

const removeTagsFromCategory = `-- name: RemoveTagsFromCategory :exec
DELETE
FROM category_tags
WHERE category_id = $1
...
type RemoveTagsFromCategoryParams struct {
	CategoryID int64   `json:"categoryId"`
	Column2    []int64 `json:"column2"`
}

func (q *Queries) RemoveTagsFromCategory(ctx context.Context, arg RemoveTagsFromCategoryParams) error {
	_, err := q.db.Exec(ctx, removeTagsFromCategory, arg.CategoryID, arg.Column2)
	return err
}
