...
with deleted as ( DELETE
    FROM records
        WHERE records.id = $1
        returning id, created_at, updated_at, amount, comment, timestamp, category_id, deleted_at, tag_id)
SELECT d.id, d.created_at, d.updated_at, d.amount, d.comment, d.timestamp, d.category_id, d.deleted_at, d.tag_id, c.type as type
FROM deleted d
...
SELECT r.id, r.created_at, r.updated_at, r.amount, r.comment, r.timestamp, r.category_id, r.deleted_at, r.tag_id, c.type as type
FROM records as r
...
WHERE c.id = records.category_id
  and records.id = $1
  and c.user_id = $2
returning records.id, records.created_at, records.updated_at, records.amount, records.comment, records.timestamp, records.category_id, records.deleted_at, records.tag_id, c.type as type
`
