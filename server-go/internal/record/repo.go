...
func (r *Repo) Create(ctx context.Context, params budget.CreateRecordParams) (ResponseDto, error) {
	record, err := r.q.CreateRecord(ctx, params)
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to create record: %w", err)
	}
	return convertToResponseDto(record), nil
}

// CreateMany создает несколько новых записей
func (r *Repo) CreateMany(ctx context.Context, list []budget.CreateRecordParams) ([]ResponseDto, error) {
...
func (r *Repo) GetByID(ctx context.Context, id int64) (ResponseDto, error) {
	...
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to get record by ID: %w", err)
	}
	return convertToResponseDto(record), nil
}
...
func (r *Repo) GetByIDAndUserID(ctx context.Context, id int64, userId int32) (ResponseDto, error) {
	...
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to get record by ID and UserID: %w", err)
	}
	return convertToResponseDto(record), nil
}

// List возвращает все записи для указанного пользователя
func (r *Repo) List(ctx context.Context, userId int32) ([]ResponseDto, error) {
	records, err := r.q.ListRecordByUserID(ctx, userId)
	if err != nil {
		return nil, fmt.Errorf("failed to list records by UserID: %w", err)
	}

	result := make([]ResponseDto, len(records))
...
func (r *Repo) ListByCategory(ctx context.Context, categoryID int64) ([]ResponseDto, error) {
...
func (r *Repo) Update(ctx context.Context, params budget.UpdateRecordParams) (ResponseDto, error) {
...
func convertToResponseDto(input any) ResponseDto {
	inVal := reflect.ValueOf(input)
	outVal := reflect.New(reflect.TypeOf(ResponseDto{})).Elem()
	...
	return outVal.Interface().(ResponseDto)
}
