...
func (r *Repo) Create(ctx context.Context, params budget.CreateCategoryParams) (ResponseDto, error) {
	category, err := r.q.CreateCategory(ctx, params)
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to create category: %w", err)
	}
...
func (r *Repo) GetByID(ctx context.Context, id int64) (ResponseDto, error) {
	...
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to get category by ID: %w", err)
	}
	return convertToResponseDto(category), nil
}

// GetByIDAndUserID получает категорию по ID и UserID
func (r *Repo) GetByIDAndUserID(ctx context.Context, id int64, userId int32) (ResponseDto, error) {
	category, err := r.q.GetCategoryByIDAndUserID(ctx, budget.GetCategoryByIDAndUserIDParams{
		ID:     id,
		UserID: userId,
	})
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to get category by ID and UserID: %w", err)
	}
	return convertToResponseDto(category), nil
}

// GetAdjustmentUserID получает категорию корректировки по UserID
func (r *Repo) GetAdjustmentUserID(ctx context.Context, userId int32) (ResponseDto, error) {
	category, err := r.q.GetAdjustmentCategoryUserID(ctx, userId)
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to get adjustment category by UserID: %w", err)
	}
	return convertToResponseDto(category), nil
}

// GetByIDAndUserIDs получает категории по списку ID и UserID
func (r *Repo) GetByIDAndUserIDs(ctx context.Context, ids []int64, userId int32) ([]ResponseDto, error) {
...
func (r *Repo) Update(ctx context.Context, params budget.UpdateCategoryParams) (ResponseDto, error) {
	category, err := r.q.UpdateCategory(ctx, params)
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to update category: %w", err)
	}
	return convertToResponseDto(category), nil
}

// UpdateMany обновляет данные нескольких категорий
func (r *Repo) UpdateMany(ctx context.Context, params []budget.UpdateCategoryParams) ([]ResponseDto, error) {
...
func (r *Repo) SoftDelete(ctx context.Context, id int64, userId int32) (ResponseDto, error) {
	category, err := r.q.SoftDeleteCategory(ctx, budget.SoftDeleteCategoryParams{
		ID:     id,
		UserID: userId,
	})
	if err != nil {
		return ResponseDto{}, fmt.Errorf("failed to soft delete category: %w", err)
	}
	return convertToResponseDto(category), nil
}

// Delete полностью удаляет категорию
func (r *Repo) Delete(ctx context.Context, id int64) (ResponseDto, error) {
...
func convertToResponseDto(c budget.Category) ResponseDto {
	return ResponseDto(c)
}

func convertListToResponseDto(list []budget.Category) []ResponseDto {
